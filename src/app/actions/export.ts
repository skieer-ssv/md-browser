...
export async function exportDirectoryToHTML(dirPath: string): Promise<string> {
    ...
    const safeDirectoryName = directoryName.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$/g, "\\$");
...
<body class="bg-neutral-950 text-neutral-200 font-sans h-screen flex overflow-hidden selection:bg-blue-500/30 selection:text-blue-200">
    ...
    <script>
        ...
        const directoryName = \`${safeDirectoryName}\`;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function renderSidebar() {
                return \`
                    <div class="transition-all duration-300 ease-in-out h-full overflow-hidden shrink-0 \${isSidebarOpen ? 'w-72 lg:w-80 border-r border-neutral-800' : 'w-0 border-r-0'}">
                        <div class="w-72 lg:w-80 bg-neutral-900/40 flex flex-col h-full shrink-0 shadow-lg z-10">
            ...
            function renderViewer() {
                ...
                const toggleBtn = \`
                    <button
                        ...
                        class="fixed bottom-6 \${isSidebarOpen ? 'left-[304px] lg:left-[336px]' : 'left-6'} z-50 p-2.5 rounded-xl bg-neutral-900/80 backdrop-blur-md border border-neutral-800 text-neutral-400 hover:text-white hover:bg-neutral-800 transition-all duration-300 shadow-2xl active:scale-95 group overflow-hidden"
                        title="\${isSidebarOpen ? 'Hide Sidebar' : 'Show Sidebar'}"
                    >
                        <div class="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        \${isSidebarOpen 
                            ? \`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><line x1="9" x2="9" y1="3" y2="21"/><path d="m14 15-3-3 3-3"/></svg>\` 
                            : \`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><line x1="9" x2="9" y1="3" y2="21"/><path d="m12 9 3 3-3 3"/></svg>\`
                        }
                    </button>
                \`;

                return viewerContent + toggleBtn;
            }
